...
NVP is a progressive web app. On Android you'll get an install prompt; on iOS,
tap Share → **Add to Home Screen** (Apple provides no automatic prompt and no
`beforeinstallprompt` event, so the app shows written instructions instead).

Once installed it runs full screen with its own icon and splash screen, and
**pass-and-play and the CPU work with no connection at all** — the AI runs
entirely in the browser, so a plane with no signal is a fine place to play the
Ace. Online rooms obviously still need a connection; the menu greys that mode out
and says so rather than letting you walk into a dead end.

Other app-like touches: haptics matched to the sound cues (Android only — iOS
Safari does not implement the Vibration API), a screen wake lock so the display
doesn't dim while you're thinking about a guess, safe-area insets for notched
devices, and home-screen shortcuts straight into CPU or pass-and-play.

### How the offline layer works

`sw.js` precaches the app shell and serves it cache-first, so a launch costs no
network at all. Three rules keep it honest:

- **`/api/*` is never cached.** A stale game state is a wrong game state, so
  online requests go to the network or fail loudly.
- **Navigations are network-first**, falling back to the cached shell, so an
  update lands as soon as one is available.
- **Updates are never applied silently.** A new version surfaces as an "Update
  ready" pill; activating a worker forces a reload, and doing that mid-match
  would cost someone their turn. It waits until you tap it.

The cache version is a hash of every cacheable file's contents, generated by
`scripts/gen-precache.mjs`. That means the version changes exactly when
something changes and never otherwise — which is what makes the update prompt
trustworthy instead of firing on every deploy. The script runs automatically
via `buildCommand` in `vercel.json`, and on `npm start`, so a forgotten manual
run can't ship a stale cache. `precache-manifest.js` is generated but committed,
so a fresh clone works without a build.
...
sw.js                offline shell, cache strategies, update handling
src/pwa.js           worker lifecycle, install prompt, connectivity state
scripts/             dev server, tests, storage checks, browser walkthroughs
```
...
`scripts/pwatest.py` covers the installable layer, which unit tests can't reach:
it confirms the worker precaches the shell, kills the server and plays a full CPU
match offline, checks the connectivity warnings, and verifies the update prompt
appears when a file changes.

`scripts/browsertest.py` drives a real browser through all three modes including
a two-context online match, and writes screenshots. Both need Playwright:
...
python3 scripts/pwatest.py       # starts and stops its own server
```
